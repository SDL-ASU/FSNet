...
from .dwsconv import Conv2d_dws, conv3x3_dws, conv1x1_dws
import pdb
...
class ResNet_imagenet(nn.Module):
    ...
    def forward(self, x):
        ...
        #import numpy
        #import scipy.io
        #scipy.io.savemat('after_relu.mat',dict(data=x[1,:].cpu().detach().numpy()))
        x = self.maxpool(x)
...
class ResNet_imagenet_dws(nn.Module):

    def __init__(self, block, layers, compression_ratio, binary_dws, num_classes=1000):
        self.inplanes = 64
        super(ResNet_imagenet_dws, self).__init__()
        self.conv1 = Conv2d_dws(3, 64, compression_ratio, kernel_size=7, \
                                      binary_dws=binary_dws, stride=2, padding=3, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0],  compression_ratio, binary_dws)
        self.layer2 = self._make_layer(block, 128, layers[1], compression_ratio, binary_dws, stride=2)
        self.layer3 = self._make_layer(block, 256, layers[2], compression_ratio, binary_dws, stride=2)
        self.layer4 = self._make_layer(block, 512, layers[3], compression_ratio, binary_dws, stride=2)
        self.avgpool = nn.AvgPool2d(7)
        self.fc = nn.Linear(512 * block.expansion, num_classes)
...
def resnet_fm18_imagenet(compression_ratio, binary_dws):
    return ResNet_imagenet_dws(BasicBlock_dws, [2, 2, 2, 2], compression_ratio, binary_dws)


def resnet_fm34_imagenet(compression_ratio, binary_dws):
    return ResNet_imagenet_dws(BasicBlock_dws, [3, 4, 6, 3], compression_ratio, binary_dws)


def resnet_fm50_imagenet(compression_ratio, binary_dws):
    return ResNet_imagenet_dws(Bottleneck_dws, [3, 4, 6, 3], compression_ratio, binary_dws)

def resnet_fm101_imagenet(compression_ratio, binary_dws):
    return ResNet_imagenet_dws(Bottleneck_dws, [3, 4, 23, 3], compression_ratio, binary_dws)

def resnet_fm152_imagenet(compression_ratio, binary_dws):
    return ResNet_imagenet_dws(Bottleneck_dws, [3, 8, 36, 3], compression_ratio, binary_dws)

def resnet18_imagenet():
    return ResNet_imagenet(BasicBlock, [2, 2, 2, 2],)


def resnet34_imagenet():
    return ResNet_imagenet(BasicBlock, [3, 4, 6, 3],)
