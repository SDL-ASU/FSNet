...
def get_model_complexity_info(model, input_res, cuda = True, print_per_layer_stat=True, as_strings=True,
    ...
    if input_constructor:
        ...
        if cuda:
           batch = torch.cuda.FloatTensor(1, *input_res)
        else:
...
def add_flops_mask(module, mask):
    def add_flops_mask_func(module):
        if isinstance(module, (Conv2d_dws,torch.nn.Conv2d)):
...
def is_supported_instance(module):
    if isinstance(module, (Conv2d_dws, torch.nn.Conv2d, torch.nn.Conv3d, torch.nn.ReLU, torch.nn.PReLU, torch.nn.ELU, \
...
def conv_dws_flops_counter_hook(conv_module, input, output):
    # Can have multiple inputs, getting the first one
    input = input[0]
    ...
    filters_per_channel = out_channels // groups
    #conv_per_position_flops = np.prod(kernel_dims) * in_channels * filters_per_channel
    kernel_size = np.prod(kernel_dims) * in_channels
    filtermap_size = conv_module.filtermap.size()[0]
    conv_per_position_flops = np.prod(kernel_dims) * in_channels  * filters_per_channel 
    ...
    r = out_channels*kernel_size / conv_module.filtermap.size()[0]
    ratio = r*kernel_dims[0]
    if kernel_dims[1] == 1:
       ratio = 1

    overall_flops = overall_flops/(ratio)
...
def add_flops_counter_hook_function(module):
    if is_supported_instance(module):
        ...
        elif isinstance(module, (Conv2d_dws)):
            handle = module.register_forward_hook(conv_dws_flops_counter_hook)
